"""
Train, evaluate, and explain fraud detection models.
Includes: Logistic Regression, Random Forest, XGBoost, LightGBM.
Handles class imbalance with SMOTE and class_weight.
Generates SHAP explanations and threshold optimization with cost analysis.
"""
...
def _get_models():
    """Return model instances with class imbalance handling."""
    models = {
        "Logistic Regression": {
    ...
    try:
        ...
        models["XGBoost"] = {
            "model": XGBClassifier(
                ...
                scale_pos_weight=49,  # ~ratio of negatives to positives for 2% fraud
            ),
            "needs_scaling": False,
        }
    except ImportError:
...
def train_and_evaluate(X_train, X_test, y_train, y_test, feature_names,
    ...
    if use_smote:
        ...
        print(f"SMOTE resampling: {X_train.shape[0]} -> {X_train_res.shape[0]} samples")
        print(f"Fraud rate after SMOTE: {y_train_res.mean():.4f}")
    else:
    ...
    for name, config in models_config.items():
        ...
        cv_scores = cross_val_score(model, Xtr, y_train_res, cv=cv, scoring="roc_auc")
...
def _generate_shap(trained_models, feature_names, X_test, X_test_scaled, best_name):
    ...
    try:
        ...
        fraud_idx = np.where(probs > 0.5)[0]
        if len(fraud_idx) > 0:
            single_idx = fraud_idx[0]
        else:
        ...
        plt.title("SHAP waterfall - single transaction")
        plt.tight_layout()
...
def _threshold_optimization(y_prob, y_test, model_name):
    ...
    Assumptions:
    - False negative cost (missed fraud): $500 average fraud loss
    - False positive cost (blocked legit): $25 customer friction + investigation
    ...
    print(f"  Recall (fraud caught): {optimal['recall']:.1%}")
    ...
    impact_df.to_csv(os.path.join(OUTPUTS_DIR, "threshold_analysis.csv"), index=False)
